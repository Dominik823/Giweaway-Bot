...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!giveawayChannel) {
            return message.channel.send(':boom: Kérlek add meg a csatornát!');
        }

        let giveawayDuration = args[1];
        if (!giveawayDuration || isNaN(ms(giveawayDuration))) {
            return message.channel.send(':boom: Kérlek add meg az időt hogy meddig tartson a nyeremény');
        }

        let giveawayNumberWinners = args[2];
        if (isNaN(giveawayNumberWinners) || (parseInt(giveawayNumberWinners) <= 0)) {
            return message.channel.send(':boom: Kérlek add meg hogy mennyi nyertes legyen.');
        }

        let giveawayPrize = args.slice(3).join(' ');
        if (!giveawayPrize) {
            return message.channel.send(':boom: Kérlek add meg a nyereményt');
        }
        if (!config["Giveaway_Options"].showMention && config["Giveaway_Options"].giveawayRoleID && config["Giveaway_Options"].giveawayMention) {
            ...
            client.giveawaysManager.start(giveawayChannel, {
                ...
                messages: {
                    ...
                    giveawayEnded: (config["Giveaway_Options"].showMention ? `<@&${config["Giveaway_Options"].giveawayRoleID}>\n\n` : "") + ":tada: **EZ A NYEREMÉNYJÁTÉK VÉGE!** :tada:",
                    timeRemaining: "Hátralévő idő: **{duration}**!",
                    inviteToParticipate: "Reagálj a 🎉 emojival hogy jelentkezz!",
                    winMessage: "Gratulálunk, {winners}! Megnyerted a nyereményt **{prize}**!",
                    embedFooter: "Giveaways",
                    noWinner: "Nincs elég jelentkező!",
                    hostedBy: "Hirdette: {user}",
                    winners: "nyertesek",
                    endedAt: "Vége lett",
                    units: {
                        seconds: "seconds",
        ...
        message.channel.send(`:tada: REMEK! A Nyereményjáték elkezdődött a  ${giveawayChannel} csatornában! <@&${config["Giveaway_Options"].giveawayRoleID}>\n\n`);
    }
}
